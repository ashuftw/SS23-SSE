...
class InputRectangleDialog(QDialog):
    ...
    def __init__(self, parent: Optional[QWidget] = None) -> None:
        ...
        layout.addRow("X", self.x_field)
        layout.addRow("Y", self.y_field)
        layout.addRow("width", self.width_field) # TASK 2
        layout.addRow("height", self.height_field) # TASK 2
        layout.addWidget(buttonBox)
...
def create_rectangle(shapes: list[Shape], command_stack: CommandStack) -> None:
    ...
    if dialog.exec():
        rectangle = Rectangle(Point(dialog.getInputs()[0], dialog.getInputs()[1]), dialog.getInputs()[2], dialog.getInputs()[3]) # TASK 2
